...
if (!in_array($participation_type, ['solo', 'group'], true))
...
elseif (strlen($event_title) > 200)
    $errors[] = 'Event title must be 200 characters or fewer.';
if ($event_date === null)
    $errors[] = 'Event date is required.';
if ($registration_date === null)
    $errors[] = 'Registration date is required.';
if (!in_array($category, $allowed_categories, true))
    $errors[] = 'Category must be Academic, Cultural, or Sports.';
if ($event_scale === 'department' && !in_array($event_scale, $allowed_scales, true))
    $errors[] = 'Event scale must be "department" or "university".';
if ($budget < 0)
    $errors[] = 'Budget must be a non-negative number.';
if ($proposed_by_id === '')
    $errors[] = 'A valid proposed_by_id is required.';
if ($event_mode === 'offline' && empty($venue))
    $errors[] = 'Venue is required for offline events.';
...
$details_arr = [];
if ($is_festival && !empty($sub_events)) {
    $details_arr['is_festival'] = true;
    $details_arr['sub_events'] = $sub_events;
}
if ($rewards !== '')
    $details_arr['rewards'] = $rewards;

$details_arr['participation_type'] = $participation_type;
if ($max_team_size !== null)
    $details_arr['max_team_size'] = $max_team_size;
if ($max_volunteers !== null)
    $details_arr['max_volunteers'] = $max_volunteers;
if ($max_coordinators !== null)
    $details_arr['max_coordinators'] = $max_coordinators;
...
if ($initial_status === 'pending_hod') {
    ...
    if ($hod_stmt) {
        ...
        if ($hod_row = $hod_stmt->get_result()->fetch_assoc()) {
            ...
            send_fcm_to_user($conn, $hod_username, "📋 Event Approval Required", $msg, $link);
        }
        $hod_stmt->close();
    }
}
